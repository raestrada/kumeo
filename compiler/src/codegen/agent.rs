...
pub fn generate_agent(agent: &Agent, output_dir: &Path, tera: &Tera) -> Result<()> {
    ...
    let (agent_type, template_dir) = match agent.agent_type.as_str() {
        "llm" | "router" => ("rust", "rust"),
        "ml" | "bayesian" => ("python", "python"),
        _ => ("rust", "rust"), // Default to Rust for unknown types
    };

    // Create agent context
    let mut context = create_base_context(&agent.id);
    ...
    let agent_dir = output_dir.join(format!("{}s/{}-{}", agent_type, agent_type, agent.id));
    ...
    process_template_dir(&template_path, &agent_dir, &context, tera)
        .with_context(|| format!("Failed to process template for agent: {}", agent.id))?;
...
fn generate_dockerfile(
    ...
    let dockerfile_template = match agent.agent_type.as_str() {
        "llm" | "router" => "rust/rust.Dockerfile.tera",
        "ml" | "bayesian" => "python/python.Dockerfile.tera",
        _ => return Ok(()), // Skip if no matching template
    };
